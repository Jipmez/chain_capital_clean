                        ...
                        @foreach($schemas as $schema)
                            ...
                            <div class="col-xxl-3 col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12">
                                <div class="single-investment-plan">
                                    ...
                                    <h3>{{$schema->name}}</h3>
                                    <p>{{ ($schema->interest_type == 'percentage' ? $schema->return_interest.'%' : $schema->return_interest.' '.$currency ) .' Every '.$schema->schedule->name .'' }}</p>
                                    <ul>
                                        ...
                                        <li>{{ __('Duration') }}
                                            <span>{{( $schema->return_type == 'period' ? $schema->number_of_period : 'Unlimited').($schema->number_of_period == 1 ? ' Time' : ' '.$schema->schedule->name .'(s)' )  }} </span>
                                        </li>
                                    </ul>
