...
class SchemaController extends Controller
{
    ...
    public function schemaSelect($id)
    {
        ...
        return [
            ...
            'return_interest' => ($schema->interest_type == 'percentage' ? $schema->return_interest.'%' : $schema->return_interest.' '.$currency ) .' Every '.$schema->schedule->name .'',
            'number_period' => ( $schema->return_type == 'period' ? $schema->number_of_period : 'Unlimited').($schema->number_of_period == 1 ? ' Time' : ' '.$schema->schedule->name .'(s)' ),
